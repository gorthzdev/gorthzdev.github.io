...
function battleModuleON() {
  ...
  current_enemy = randomizeMobs(); //console.log(current_enemy, 'Mob random')
    ...
    //lista de mobs para agregar al localstorage;
     registerMonster(current_enemy)
     console.log(mobRegister)//
}
...
function registerMonster(monster){
 let existe = false;
  ...
  try{
   mobRegister = [...JSON.parse(localStorage.getItem('Mobs'))]
  }catch(error){
     console.error(error);
  }
  
 if(mobRegister === null){
    mobRegister.push(monster);
    existe =  true;
  //  mobRegister += [...parsedData];
 }else{
    mobRegister.forEach(mob=>{
      mob.name === monster.name ? existe = true: '';
    })
  
  }
  if(!existe){ mobRegister.push(monster)}
  
  console.log(mobRegister , 'mobreg');
  localStorage.setItem('Mobs', JSON.stringify(mobRegister));
}
